...
cswap auto --start-timer-on-reset  # prime each account's next 5h window
...
- **Start five-hour timers early** (opt-in): a subscription's next 5h clock starts with its first message, not merely when the old reset time passes. Run `cswap auto --start-timer-on-reset`, or persist it with `cswap config set autoswitch.startTimerOnReset true`. After a fresh usage poll proves an enabled OAuth account rolled over, claude-swap sends exactly one tiny, tool-free, non-persistent Claude Code prompt as that account without changing your active login. An already-idle 0% window with no reset timestamp is primed immediately; other first observations only establish a baseline. After a successful prompt, that account is refetched so the shared TUI and menu-bar cache show the new countdown immediately. A persistent claim prevents duplicate prompts across the CLI, TUI, menu bar, and cron, and failures retry after five minutes. Disabled, quarantined, and API-key accounts are skipped. This consumes a small amount of the newly available quota; `--dry-run` reports `would-start` without sending anything.
...
Session-mode profiles (`cswap run`) live under the backup directory in `sessions/`. Tool preferences (`settings.json`) and auto-switch state (`autoswitch_state.json` — cooldown, quarantined accounts, and five-hour timer reset claims; delete it to reset) live in the backup directory root.
...
cswap config set autoswitch.startTimerOnReset true  # prime new 5h windows
...
`cswap auto --json` emits an event *stream* instead — one JSON object per line (`{"schemaVersion":1,"event":"switch","ts":…, …}` with kinds like `poll`, `switch`, `no-switch`, `five-hour-timer`, `account-quarantined`, `all-exhausted`, `error`). The contract is additive: new kinds and fields may appear, so scripts should ignore unknown ones.

...
- **Fallback account** (`--fallback-account <num|email|alias>`, or `cswap config set autoswitch.fallbackAccount enterprise`): once the active account can no longer carry on and every other OAuth candidate is a dead end too, force onto one designated account instead of sitting blocked until the earliest reset. "Can no longer carry on" covers two states: literally out of quota (or credential dead), or — with real headroom left on every side — the active account and every other OAuth candidate are over the switch threshold, and the fallback itself (exempt from that threshold check) is known to hold no less headroom than the active account right now. That second form is the common case: two accounts both past the threshold but neither literally spent, where the fallback is designated precisely so this doesn't sit blocked hoping for a landing spot that never clears the threshold. Normal rotation resumes as soon as any account recovers headroom. Off by default, and it never preempts an ordinary switch: while the active account is still below the switch threshold, or any candidate is worth moving to, this changes nothing. An API-key account can't be the fallback, in either setting of `includeApiKeyAccounts`, and a warning says so: with it off, auto-switch would park on the metered account and never rotate off it; with it on, auto-switch already reaches for API-key accounts by itself as a last resort, in rotation order rather than by your designation.
- **Strategies** (`--strategy`, or `cswap config set autoswitch.strategy`): `best` (default) stays put until the active account nears its limit, then moves to the account with the most quota left. `consume-first` proactively keeps you on the account whose **weekly window resets soonest** — use-it-or-lose-it — switching to a sooner-resetting account (with room to spare) even below the threshold, so perishable weekly quota isn't wasted. `priority` follows a rank-ordered list you set instead of chasing headroom: `--priority-accounts <num|email|alias>,...` (or `cswap config set autoswitch.priorityAccounts enterprise,max`). While the active account is below the threshold, it recalls to the highest-ranked entry that's below the threshold too — *even though the active, lower-ranked account is itself still healthy*. At or above the threshold the ordinary headroom ranking decides, so a rank order can never strand you on an exhausted account: `at-limit`, `failover` and `--fallback-account` all still see every candidate. Every entry is capped, the last one included — recall departs a healthy account, so landing on a spent one would only bounce straight back. An unlisted account ranks below every listed one; a quarantined entry is skipped for the next one down; recall respects `--cooldown-seconds`. An identifier that can't be recalled to, or that duplicates one ranked higher, is dropped with a one-time warning; an empty list warns that `priority` behaves exactly like `best`.

...
- **Strategies** (`--strategy`, or `cswap config set autoswitch.strategy`): `best` (default) stays put until the active account nears its limit, then moves to the account with the most quota left. `consume-first` proactively keeps you on the account whose **weekly window resets soonest** — use-it-or-lose-it — switching to a sooner-resetting account (with room to spare) even below the threshold, so perishable weekly quota isn't wasted.
  - Under `consume-first` the threshold is **not** the switch point: the active account is burned all the way to its limit, because the last few percent of a weekly window are exactly what expires unused at the reset. The threshold still bars *landing* on an account that's nearly spent, and an account at 100% still escapes immediately to any account with room. (Under `best` it keeps its usual meaning — the line at which the engine starts looking.)
...
- **Resuming limit-stopped sessions** (`cswap config set autoswitch.resumeStoppedSessions true`, off by default): when every account is exhausted, any Claude Code session that was working stops with a rate-limit message and stays stopped — the window resets hours later, cswap switches to the recovered account, and the session sits idle until you notice. With this on, cswap remembers the sessions it saw stop and nudges them as soon as there's quota to work with again — whether that's because it switched to a recovered account or because the account you were already on had its own window roll over — so they pick up where they left off. While auto-switching, it only touches sessions it *witnessed* stopping: one that stopped before cswap started, or that you already resumed by hand, is left alone. Switching by hand (`cswap use`, `cswap switch`, the menu bar, or the TUI) has no such history to go on, so it wakes any live session still sitting on a limit message — which is what you want if you turned auto-switch off and are driving the switches yourself. Also togglable from the [menu bar](#menu-bar-macos) (*Resume limit-stopped sessions*).
  - **macOS and Linux only.** Claude Code exposes this channel as a Unix socket on POSIX and as a named pipe on Windows; cswap speaks the former. On Windows the setting can be set but nothing is ever resumed.
  - Two caveats worth knowing. A resumed session **continues on its own** — it re-reads its transcript and carries on with whatever it was doing, unattended, spending quota; that is the point of the feature, but it means you should only enable it if unattended continuation is what you want. And it works by talking to Claude Code over an undocumented local channel, so a Claude Code update can disable it; cswap checks the protocol version and, if it doesn't recognise it, logs a warning and resumes nothing rather than guessing.

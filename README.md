...
- **Per-window lines** (`cswap config set autoswitch.thresholdWeekly 95`, `autoswitch.thresholdFiveHour`): the two windows want opposite values. Quota not spent before a weekly reset is gone for good, so that window wants an aggressive line; overshooting the 5h window costs one interrupted turn and it recycles in hours. Each window is measured against its own line and the closest one decides. Unset, both fall back to `autoswitch.threshold` and nothing changes.

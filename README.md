...
- **Fallback account** (`--fallback-account <num|email|alias>`, or `cswap config set autoswitch.fallbackAccount enterprise`): once the active account and every candidate are exhausted, force onto one designated account — a separately-paid seat, say — instead of sitting blocked until the earliest reset. Normal rotation resumes as soon as any account recovers headroom. Off by default, and it never preempts an ordinary switch: while the active account still has quota, or any candidate does, nothing changes. API-key accounts qualify only with `includeApiKeyAccounts` on, since auto-switch would otherwise park on a metered account and never rotate off it.

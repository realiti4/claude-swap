...
- **Strategies** (`--strategy`, or `cswap config set autoswitch.strategy`): `best` (default) stays put until the active account nears its limit, then moves to the account with the most quota left. `consume-first` proactively keeps you on the account whose **weekly window resets soonest** — use-it-or-lose-it — switching to a sooner-resetting account (with room to spare) even below the threshold, so perishable weekly quota isn't wasted. `balance` picks the account that is furthest **behind an even weekly schedule** — so every account's weekly quota gets spent shortly before its own reset instead of one account being drained while the rest sit idle, and several accounts keep 5-hour headroom at the same time. It switches when `best` does, and once besides: a default login whose every session has been idle long enough moves on the score margin alone, without waiting for the threshold.
  - **How `balance` scores:** among the accounts that clear the usual health checks — below the threshold, better than the current account by the hysteresis margin, and not the one you just switched away from — each account's on-schedule target is `min(100, 100 × elapsed / (7 days − lead))`, where `elapsed` is the time since its weekly window started; `slack = target − weekly %` (positive = behind schedule). An account is then set aside if its projected 5-hour usage would reach `autoswitch.balanceFiveHourCeiling` (default 85), or its weekly or `--model` window is already at the threshold; whatever remains is ranked by `slack − balanceFiveHourWeight × projected 5-hour %`, highest first. If nothing qualifies, `balance` falls back to whichever surviving account's limiting window comes back soonest, so it never refuses to move while any account still has room. The cooldown and the "every account above the threshold" fallback work the same as for `best`, and so does the threshold for every move but one: `balance` also moves a default login that is **still below the threshold**, when every Claude Code session running on `~/.claude` has been idle for `sessions.idleReassignMinutes` and another account is `sessions.reassignMargin` ahead on the balance score. That score margin is what admits the idle move, so the hysteresis margin above is not applied to it as well — an idle login has no conversation whose prompt cache the move would spend (see [balanced managed sessions](#balanced-managed-sessions-cswap-run---auto)).
...
Those two settings cover your default login as well: with `autoswitch.strategy = balance`, when every Claude Code session running on `~/.claude` has been idle for `sessions.idleReassignMinutes` and another account scores `sessions.reassignMargin` better, `cswap auto` moves the default login too, rather than waiting for it to near the threshold — it never interrupts a conversation to rebalance.

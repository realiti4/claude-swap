...
- By default only the account-wide 5h/7d windows drive switching. If you work on one model and hit its **weekly per-model limit** first (e.g. Fable), add `--model Fable` (or `cswap config set autoswitch.model Fable`) to fold that model's window into the decision, so it switches off an account whose model quota is spent even while its 5h/7d windows still have room.
  ...
  - **An account that reports no window for the configured model is not a switch target** (it shows as `skipped: model-window-missing` in the poll line and as `skippedCandidates` in `--json`), the same way a re-login-required account is skipped: an account that cannot serve the model looks exactly like one with plenty of room on its 5h/7d windows, and landing on it makes Claude Code offer a fallback model. An account whose usage is unreadable this tick stays "unknown", never "missing". If the *active* account stops reporting the window on `autoswitch.unhealthyTicks` consecutive fresh reads (default 3 — never on a single read), it is treated as at its limit for that model and left for an account that does report it (`model-window-lost` event). A bare `all` names no model and never makes an account ineligible.

...
- **Start five-hour timers early** (opt-in): a subscription's next 5h clock starts with its first message, not merely when the old reset time passes. Run `cswap auto --start-timer-on-reset`, or persist it with `cswap config set autoswitch.startTimerOnReset true`. After a fresh usage poll proves an enabled OAuth account rolled over, claude-swap sends exactly one tiny, tool-free, non-persistent Claude Code prompt as that account without changing your active login. The first observation only establishes a baseline; a persistent claim prevents duplicate prompts across the CLI, TUI, menu bar, and cron, and failures retry after five minutes. Disabled, quarantined, and API-key accounts are skipped. This consumes a small amount of the newly available quota; `--dry-run` reports `would-start` without sending anything.

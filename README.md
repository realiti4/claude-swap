...
- **Strategies** (`--strategy`, or `cswap config set autoswitch.strategy`): `best` (default) stays put until the active account nears its limit, then moves to the account with the most quota left. `consume-first` proactively keeps you on the account whose **weekly window resets soonest** — use-it-or-lose-it — switching to a sooner-resetting account (with room to spare) even below the threshold, so perishable weekly quota isn't wasted.
  - It's **burn-rate aware**: weekly quota can only be drained *through* the 5-hour window, so an account can reset soonest yet still hold more quota than you could realistically consume before it resets. Once it has measured how fast the active account burns (Δ weekly-% over time), `consume-first` skips a proactive move onto a soonest-resetting account whose remaining quota is structurally unreachable in the time left, preferring the next-soonest account you can actually finish — and holds put (rather than spending a cooldown for nothing) when the imminent window can't be meaningfully drained. Until it has a rate to work from it degrades to plain soonest-first, so early behavior is unchanged.

...
def test_launch_terminal_invokes_osascript_with_argument_list():
    executable = "/Applications/Claude Swap/cswap"
    with patch("claude_swap.macos_terminal.subprocess.run") as run:
        run.return_value = _completed(0)

        result = macos_terminal.launch_terminal(4, cswap_executable=executable)

    assert result.launched is True
    assert result.slot == "4"
    assert result.command == "'/Applications/Claude Swap/cswap' run 4"
...
def test_launch_terminal_returns_structured_applescript_error():
    with patch("claude_swap.macos_terminal.subprocess.run") as run:
...
def test_launch_terminal_returns_structured_missing_osascript_error():
    with patch(
...
def test_launch_terminal_returns_structured_timeout_error():
...
def test_launch_terminal_returns_structured_os_error():
    with patch(
        "claude_swap.macos_terminal.subprocess.run",

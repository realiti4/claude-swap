...
class TestFindAccountSlot:
    ...
    def test_matches_composite_identity(self):
        assert (
            ClaudeAccountSwitcher._find_account_slot(self.DATA, "user@example.com", "org-123")
            == "2"
        )

    def test_same_email_wrong_org_is_no_match(self):
        assert (
            ClaudeAccountSwitcher._find_account_slot(self.DATA, "user@example.com", "org-999")
            is None
        )

    def test_absent_email_is_no_match(self):
        assert ClaudeAccountSwitcher._find_account_slot(self.DATA, "nobody@example.com", "") is None
    ...
    def test_empty_org_matches_missing_or_empty_org_field(self):
        ...
        assert ClaudeAccountSwitcher._find_account_slot(self.DATA, "user@example.com", "") == "1"
        assert ClaudeAccountSwitcher._find_account_slot(self.DATA, "other@example.com", "") == "3"
...
class TestStatusCache:
    ...
    def test_status_fetches_with_is_active_true_when_cc_running(
        ...
        entry = UsageStore(switcher.backup_dir / "cache").entries({"1": ("test@example.com", "")})[
            "1"
        ]
    ...
    def test_status_preserves_other_accounts_in_cache(
        ...
        entries = store.entries({"1": ("test@example.com", ""), "2": ("account2@example.com", "")})
...
class TestFetchAccountUsageSessionProfile:
    ...
    def test_fresh_session_credentials_fetch_read_only(self, temp_home: Path):
        ...
        with (
            patch.object(switcher, "_live_session_pids", return_value=[123]),
            patch("claude_swap.session.read_session_credentials", return_value=session),
            patch(
                "claude_swap.oauth.try_fetch_usage_for_account",
    ...
    def test_expired_session_credentials_with_live_session_is_sentinel(self, temp_home: Path):
    ...
    def test_expired_session_credentials_without_live_session_falls_back(self, temp_home: Path):
...
class TestListAccountsUsage:
    ...
    def test_list_refetches_stale_entries(
        ...
        backdated = UsageStore(switcher.backup_dir / "cache", clock=lambda: time_mod.time() - 100)
        backdated.record(
...
class TestActiveAccountRefresh:
    ...
    def test_detection_failure_fails_closed(
        ...
        with patch("claude_swap.switcher.get_running_instances", side_effect=OSError("boom")):
...
class TestPerformSwitchPostDisplay:
    ...
    def test_switch_with_unset_active_account_does_not_write_none_backup(
        ...
        assert json.loads(live_state["creds"])["claudeAiOauth"]["accessToken"] == ("target-token")
        data = switcher._get_sequence_data()
...
class TestSwitchToSelfSlotAndForce:
    ...
    _install_store_patches = staticmethod(TestPerformSwitchPostDisplay._install_store_patches)
...
class TestUpgradeMigration:
    ...
    def test_list_after_upgrade_marks_active(self, temp_home, sample_sequence_data_pre_v06, capsys):
    ...
    def test_migration_uses_live_config_over_backup(self, temp_home, sample_sequence_data_pre_v06):
    ...
    def test_migration_idempotent(self, temp_home, sample_sequence_data_pre_v06):
        ...
        assert (
            data1["accounts"]["1"]["organizationUuid"] == data2["accounts"]["1"]["organizationUuid"]
        )
        assert (
            data1["accounts"]["2"]["organizationUuid"] == data2["accounts"]["2"]["organizationUuid"]
        )

    def test_migration_skips_already_migrated(self, temp_home, sample_sequence_data_pre_v06):
...
class TestAddAccountFromToken:
    ...
    def test_credentials_blob_format(self, temp_home):
        ...
        with (
            patch.object(switcher, "_write_account_credentials", side_effect=capture_creds),
            patch.object(switcher, "_write_account_config"),
        ):
...
class TestSwitchSkipsBrokenSlots:
    ...
    def test_rotation_no_valid_targets_returns_without_error(self, temp_home: Path, capsys):
...
class TestUsageAwareSwitch:
    ...
    def test_best_current_exhausted_with_unknown_candidate_stays(self, temp_home: Path, capsys):
...
class TestClaudeCodeLockCooperation:
    ...
    def test_switch_holds_both_cc_locks_at_write_time(self, temp_home: Path):
        ...
        with (
            patch.object(s, "_write_credentials", side_effect=spying_write),
            patch.object(s, "list_accounts"),
        ):
            s.switch_to("2")
    ...
    def test_preheld_cc_lock_fails_cleanly_without_mutation(self, temp_home: Path, monkeypatch):
        ...
        live_creds_before = (temp_home / ".claude" / ".credentials.json").read_text()
        with pytest.raises(ClaudeCodeLockTimeout):
            s.switch_to("2")

        # Nothing was mutated: locks acquire before any write.
        assert s._get_sequence_data()["activeAccountNumber"] == 1
        live_creds_after = (temp_home / ".claude" / ".credentials.json").read_text()
...
class TestMacosKeychainFallback:
    ...
    def test_item_exists_is_capability_neutral(self, temp_home: Path, block_real_keychain):
        s = self._macos_switcher()
    ...
    def test_active_write_does_not_swallow_programming_errors(self, temp_home: Path, monkeypatch):
    ...
    def test_active_read_prefers_keychain_then_file(self, temp_home: Path, block_real_keychain):
        s = self._macos_switcher()
        acct = macos_keychain.keychain_account_name()
    ...
    def test_keychain_write_refreshes_existing_file(self, temp_home: Path, block_real_keychain):
    ...
    def test_keychain_write_does_not_create_absent_file(self, temp_home: Path, block_real_keychain):
    ...
    def test_backup_read_enc_wins_over_stale_keychain(self, temp_home: Path, block_real_keychain):
        s = self._macos_switcher()
    ...
    def test_backup_keychain_write_deletes_enc(self, temp_home: Path, block_real_keychain):
        s = self._macos_switcher()
    ...
    def test_backup_bad_enc_falls_back_to_keychain(self, temp_home: Path, block_real_keychain, bad):
    ...
    def test_backup_delete_removes_both_backends(self, temp_home: Path, block_real_keychain):
        s = self._macos_switcher()
    ...
    def test_healthy_mac_reads_create_no_files(self, temp_home: Path, block_real_keychain):
        s = self._macos_switcher()
        s._kc_write_backup("1", "a@example.com", "KC")

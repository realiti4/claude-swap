...
Built on ``rumps`` (an optional extra, macOS only); ``rumps`` is imported
lazily inside the app glue. The pure helpers (formatting, log parsing, the
v2 panel view-model) now live in ``claude_swap.menubar.viewmodel`` and are
imported here for the legacy glue.
"""

...
def run_login_flow(*, open_browser: bool = True, private: bool = False) -> LoginResult:
    ...
    if open_browser and private:
        label = _open_private_browser(url)
        if label:
            print(muted(f"Opened a private {label} window (your normal session stays signed in)."))
        else:
            warning(
        ...
        try:
            webbrowser.open(url)
        except Exception:  # noqa: BLE001 — headless/no-browser is fine, URL is printed
            pass
    ...
    code, returned_state = _parse_pasted_code(pasted, state)
    if not code:
        raise LoginError("Could not read an authorization code from that input.")

    resp = _exchange_code(code, returned_state, verifier)
